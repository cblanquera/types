...
class Args {
   ...
   * The main hash
   */
  public hash: HashStore;

  /**
   * Sets the hash 
   */
  constructor(hash: HashStore) {
    this.hash = hash;
  }
...
class Path {
   ...
   */
  remove(notation: string, separator: string = '.'): HashStore {
    const path = notation.split(separator);
   ...
   * and sets the value to that name space
   */
  set(notation: string, value: any, separator: string = '.'): HashStore {
    const path = notation.split(separator);
